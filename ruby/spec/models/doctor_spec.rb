...
RSpec.describe Doctor, type: :model do
  ...
  context '#find_by_id' do
    ...
    it 'return nil when it does not exist in database' do
      ...
      result = Doctor.find_by_id(99_999, @conn)
  ...
  context '#exams' do
    it 'return array with the instance doctor exams' do
      ...
      other_doctor = Doctor.create(
        { crm: '248624', crm_state: 'MA', name: 'Maria Luisa Gonçalvez', email: 'mlgon@email.com' }, @conn
      )
      ...
      Exam.create({ token: 'PGIE13', date: Date.new(2024, 0o1, 18), patient_id: patient.id, doctor_id: doctor.id },
                  @conn)
      Exam.create({ token: 'N2GJ', date: Date.new(2024, 0o2, 0o3), patient_id: patient.id, doctor_id: other_doctor.id },
                  @conn)
