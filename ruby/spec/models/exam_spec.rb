...
RSpec.describe Exam, type: :model do
  context '#all' do
    it 'return array with every instance in database' do
      ...
      Exam.create({token: 'N213GJ', date: Date.new(2024, 02, 03), patient_id: patient.id, doctor_id: doctor.id}, @conn)
  ...
  context '#find_by_id' do
    it 'return object when it exists in database' do
      ...
      expect(result.class).to eq Exam
      expect(result.token).to eq 'IG4O21'
    ...
    it 'return nil when it does not exist in database' do
      ...
      result = Exam.find_by_id(99999, @conn)
  ...
  context '#create' do
    it 'return object when it is created in database' do
      ...
      expect(exam.token).to eq 'IG4O21'
  ...
  context '#tests' do
    ...
    it 'return empty array when there are no tests' do
      ...
      doctor = Doctor.create({crm: '654321', crm_state: 'CE', name: 'Leonardo Silva', email: 'leo@email.com'}, @conn)
      exam = Exam.create({token: 'IG4O21', date: Date.new(2024, 01, 12), patient_id: patient.id, doctor_id: doctor.id}, @conn)
      no_test_exam = Exam.create({token: 'PGIE13', date: Date.new(2024, 01, 18), patient_id: patient.id, doctor_id: doctor.id}, @conn)
